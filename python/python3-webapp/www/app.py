...
def auth_factory(app, handler):
    ...
    def auth(request):
        ...
        if request.path.startswith('/manage') and (request.__user__ is None or not resuest.__user__.admin):
...
@asyncio.coroutine
def init(loop):
    yield from orm.create_pool(loop=loop, host='127.0.0.1', port=3306, user='root', password='a111111', db='awesome')
